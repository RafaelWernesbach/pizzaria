...
O arquivo \texttt{graph.py} j\'a automatiza os testes de desempenho exatamente nas combina\c{c}\~oes pedidas no PDF apresentado pelo professor. Na vers\~ao atual, o script executa as configura\c{c}\~oes \(1{+}1\), \(2{+}2\), \(4{+}4\) e \(8{+}8\), sempre somando chefs e entregadores para calcular o total de threads analisado nos gr\'aficos. Para que a compara\c{c}\~ao de speedup seja justa, o experimento mant\'em fixa a carga total de trabalho em 120 pizzas, distribuindo essa quantidade entre os chefs de cada teste.
...
\clearpage
\section{Ap\^endice -- C\'odigos}
\subsection{\texttt{pizzaria.py}}
Como \texttt{pizzaria.py} concentra a l\'ogica central da aplica\c{c}\~ao, esta se\c{c}\~ao apresenta o seu c\'odigo-fonte completo. Nele est\~ao reunidos os mecanismos de sincroniza\c{c}\~ao, as estruturas compartilhadas, a l\'ogica das threads produtoras e consumidoras e a fun\c{c}\~ao respons\'avel por coordenar toda a simula\c{c}\~ao.

{\footnotesize
\verbatiminput{pizzaria.py}
}

\subsection{\texttt{interface.py}}
O arquivo \texttt{interface.py} implementa a interface gr\'afica usando Tkinter. Ele define a classe \texttt{Interface}, que exibe o estado dos chefs, dos entregadores e as estat\'isticas gerais da simula\c{c}\~ao.
{\footnotesize
\verbatiminput{interface.py}
}

\subsection{\texttt{main.py}}
O arquivo \texttt{main.py} coordena a execu\c{c}\~ao da simula\c{c}\~ao, inicializando os componentes e gerenciando o fluxo de trabalho.
{\footnotesize
\verbatiminput{main.py}
}

\subsection{\texttt{graph.py}}
O arquivo \texttt{graph.py} viabiliza a an\'alise de desempenho.
{\footnotesize
\verbatiminput{graph.py}
}
