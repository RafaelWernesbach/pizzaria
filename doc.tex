...
\author{
Nome do aluno 1: \rule{7cm}{0.4pt}\\
Matr\'icula: \rule{5cm}{0.4pt}\\[0.5em]
Nome do aluno 2: \rule{7cm}{0.4pt}\\
Matr\'icula: \rule{5cm}{0.4pt}
}
\date{\today}
...
O arquivo \texttt{graph.py} j\'a automatiza os testes de desempenho exatamente nas combina\c{c}\~oes pedidas no PDF. Na vers\~ao atual, o script executa as configura\c{c}\~oes \(1{+}1\), \(2{+}2\), \(4{+}4\) e \(8{+}8\), sempre somando chefs e entregadores para calcular o total de threads analisado nos gr\'aficos.
...
\begin{itemize}
    \item a quantidade de pizzas por chef foi fixada em 10;
...
\placeholderfigure{Execu\c{c}\~ao geral do sistema mostrando chefs, entregadores e a interface gr\'afica em funcionamento.}{6cm}

\placeholderfigure{Situa\c{c}\~ao em que chefs est\~ao aguardando o forno, evidenciando o bloqueio causado pelo sem\'aforo.}{6cm}

\placeholderfigure{Situa\c{c}\~ao em que uma pizza fica pronta e ocorre a notifica\c{c}\~ao do consumidor via \texttt{Condition}.}{6cm}
...
Al\'em da exibi\c{c}\~ao em janela quando o backend do \texttt{matplotlib} permite, o script salva automaticamente a imagem do resultado em \texttt{grafico\_pizzaria.png}. Isso facilita a inclus\~ao do gr\'afico no relat\'orio mesmo em ambientes onde a interface gr\'afica do \texttt{matplotlib} n\~ao abre corretamente.
...
\subsection{Tabela para preenchimento dos resultados}
\begin{table}[H]
\centering
\begin{tabular}{cccc}
\toprule
...
\midrule
1 chef + 1 entregador & 2 & \rule{2.5cm}{0.4pt} & \rule{2cm}{0.4pt} \\
2 chefs + 2 entregadores & 4 & \rule{2.5cm}{0.4pt} & \rule{2cm}{0.4pt} \\
4 chefs + 4 entregadores & 8 & \rule{2.5cm}{0.4pt} & \rule{2cm}{0.4pt} \\
8 chefs + 8 entregadores & 16 & \rule{2.5cm}{0.4pt} & \rule{2cm}{0.4pt} \\
\bottomrule
\end{tabular}
\caption{Tabela para registro dos tempos e speedups.}
\end{table}

\placeholderfigure{Inserir aqui o gr\'afico de tempo total e speedup gerado a partir do arquivo \texttt{graph.py}, preferencialmente usando o arquivo \texttt{grafico\_pizzaria.png}.}{7cm}
...
\subsection{Roteiro de resposta}
Ap\'os preencher a tabela da se\c{c}\~ao anterior, esta se\c{c}\~ao pode ser completada com base nos resultados experimentais:
\begin{enumerate}
    \item verificar se o speedup continuou crescendo de forma aproximadamente linear;
    \item estimar a fra\c{c}\~ao sequencial \(B\) usando dois pontos da tabela;
    \item discutir qual seria o speedup m\'aximo te\'orico em uma m\'aquina com 32 n\'ucleos;
    \item apontar custos reais observados, como GIL, espera pelo sem\'aforo, conten\c{c}\~ao na fila e atualiza\c{c}\~ao de estado compartilhado.
\end{enumerate}

\subsection{Espa\c{c}o para an\'alise}
\noindent\textbf{Resposta sugerida para ser completada ap\'os a execu\c{c}\~ao dos testes:}

\vspace{0.5em}
\noindent\framebox[\textwidth]{
\parbox[c][8cm][t]{0.95\textwidth}{
\vspace{0.5em}
Escrever aqui a an\'alise quantitativa da Lei de Amdahl com base na tabela obtida, discutindo o comportamento do speedup, a estimativa de \(B\) e os fatores reais de overhead observados na implementa\c{c}\~ao.
}
}
...
\subsection{Espa\c{c}o para resposta final}
\noindent\framebox[\textwidth]{
\parbox[c][7cm][t]{0.95\textwidth}{
\vspace{0.5em}
Escrever aqui a reflex\~ao final do grupo, abordando a principal li\c{c}\~ao aprendida sobre os limites reais do paralelismo e comentando em quais tipos de sistemas reais esse tipo de satura\c{c}\~ao do speedup se torna mais cr\'itico.
}
}
